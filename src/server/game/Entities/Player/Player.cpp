...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options, GameObject* go)
{
        ...
        if (pProto)
        {
            ...
            if (not_loading)
            {
                ...
                if (!pProto->CanChangeEquipStateInCombat())
                {
					...
					if (HasAura(88011) || HasAura(88009))
...
//                            break;
						...
						case ITEM_MOD_ROLE_STR: // ROLE STAT SYSTEM
							SetRoleStat(ROLE_STAT_STR, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_AGI:
							SetRoleStat(ROLE_STAT_AGI, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_INT:
							SetRoleStat(ROLE_STAT_INT, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_VIT:
							SetRoleStat(ROLE_STAT_VIT, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_DEX:
							SetRoleStat(ROLE_STAT_DEX, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_WILL:
							SetRoleStat(ROLE_STAT_WILL, enchant_amount, apply);
							break;
						case ITEM_MOD_ROLE_SPI:
							SetRoleStat(ROLE_STAT_SPI, enchant_amount, apply);
							break;
                        case ITEM_MOD_ROLE_CHARISM:
                            SetRoleStat(ROLE_STAT_CHARISM, enchant_amount, apply);
                        break;
                        case ITEM_MOD_ROLE_AVOID:
                            SetRoleStat(ROLE_STAT_AVOID, enchant_amount, apply);
                        break;
                        case ITEM_MOD_ROLE_LUCK:
                            SetRoleStat(ROLE_STAT_LUCK, enchant_amount, apply);
                        break;
                        case ITEM_MOD_ROLE_HIDDEN:
                            SetRoleStat(ROLE_STAT_HIDDEN, enchant_amount, apply);
                        break;
                        case ITEM_MOD_ROLE_INIT:
                            SetRoleStat(ROLE_STAT_INIT, enchant_amount, apply);
                        break;
                        case ITEM_MOD_ROLE_PER:
                            SetRoleStat(ROLE_STAT_PER, enchant_amount, apply);
                        break;
...
void Player::_LoadRoleStats(PreparedQueryResult result) // ROLE STAT SYSTEM
{
	if (result)
	{
		do
		{
			...
			SetRoleStat(ROLE_STAT_STR, fields[0].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_AGI, fields[1].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_INT, fields[2].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_VIT, fields[3].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_DEX, fields[4].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_WILL, fields[5].GetUInt32(), true, false);
			SetRoleStat(ROLE_STAT_SPI, fields[6].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_CHARISM, fields[7].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_AVOID, fields[8].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_LUCK, fields[9].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_HIDDEN, fields[10].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_INIT, fields[11].GetUInt32(), true, false);
            SetRoleStat(ROLE_STAT_PER, fields[12].GetUInt32(), true, false);
...
void Player::TextEmote(std::string const& text, WorldObject const* /*= nullptr*/, bool /*= false*/)
{
    ...
    //SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), true, !GetSession()->HasPermission(rbac::RBAC_PERM_TWO_SIDE_INTERACTION_CHAT)); Miton
	SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), true, false);
}
...
void Player::VehicleSpellInitialize()
{
    ...
    for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i)
    {
        ...
        if (!sConditionMgr->IsObjectMeetingVehicleSpellConditions(vehicle->GetEntry(), spellId, this, vehicle))
        {
			...
			TC_LOG_ERROR("Entity:Player", "Player::VehicleSpellInitialize: Player '%s' (%s) doesn't meet conditions for vehicle (Entry: %u, Spell: %u)",
				GetName().c_str(), GetGUID().ToString().c_str(), vehicle->ToCreature()->GetEntry(), spellId);
            data << uint16(0) << uint8(0) << uint8(i+8);
            continue;
        }

...
namespace LuaGlobalFunctions
{
    ...
    int PerformIngameSpawn(Eluna* /*E*/, lua_State* L)
    {
		...
		//Creature* creatureReplacer = Eluna::CHECKOBJ<Creature>(L, 13, NULL);
#ifndef TRINITY
        Map* map = eMapMgr->FindMap(mapID, instanceID);
        if (!map)
        {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
				...
				//creature->SetCreatureReplace(creatureReplacer);
    ...
    int ReloadCreatureOutfitByEntry(Eluna* /*E*/, lua_State* L)
    {
        uint32 entry = Eluna::CHECKVAL<uint32>(L, 1);
        sObjectMgr->ReloadCreatureOutfit(entry);
        return 1;
    }

...
enum Races
{
    ...
    RACE_FEL_ORC            = 12,
    RACE_NAGA               = 13, // ворген
    RACE_BROKEN             = 14, // пандарен
    RACE_SKELETON           = 15, // войдэльф
    RACE_VRYKUL             = 16, // озаренный дреней
    RACE_TUSKARR            = 17, // вульпера
...
#define RACEMASK_ALL_PLAYABLE \
    ((1<<(RACE_HUMAN-1))    |(1<<(RACE_ORC-1))                |(1<<(RACE_DWARF-1))     | \
	 ...
	 (1<<(RACE_BLOODELFN-1))  |(1<<(RACE_THINHUMAN-1))       |(1<<(RACE_UPRIGHT-1))    | \
	 (1<<(RACE_HALFELF-1)))

#define RACEMASK_ALLIANCE \
    ((1<<(RACE_HUMAN-1)) | (1<<(RACE_DWARF-1)) | (1<<(RACE_NIGHTELF-1)) | \
     (1<<(RACE_GNOME-1)) | (1<<(RACE_GOBLIN-1))	| (1<<(RACE_BLOODELF-1)) | \

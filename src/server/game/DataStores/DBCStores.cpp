...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    for (uint32 i = 0; i < sNamesProfanityStore.GetNumRows(); ++i)
    {
        ...
        if (namesProfanity->Language != -1)
            NamesProfaneValidators[namesProfanity->Language].emplace_back(wname, boost::regex::perl | boost::regex::icase | boost::regex::optimize);
        else
            for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
                NamesProfaneValidators[i].emplace_back(wname, boost::regex::perl | boost::regex::icase | boost::regex::optimize);
    }
    ...
    for (uint32 i = 0; i < sNamesReservedStore.GetNumRows(); ++i)
    {
        ...
        if (namesReserved->Language != -1)
            NamesReservedValidators[namesReserved->Language].emplace_back(wname, boost::regex::perl | boost::regex::icase | boost::regex::optimize);
        else
            for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
                NamesReservedValidators[i].emplace_back(wname, boost::regex::perl | boost::regex::icase | boost::regex::optimize);
    }
...
ResponseCodes ValidateName(std::wstring const& name, LocaleConstant locale)
{
    ...
    for (boost::wregex const& regex : NamesProfaneValidators[locale])
        if (boost::regex_search(name, regex))
            return CHAR_NAME_PROFANE;

    // regexes at TOTAL_LOCALES are loaded from NamesReserved which is not locale specific
    for (boost::wregex const& regex : NamesReservedValidators[locale])
        if (boost::regex_search(name, regex))

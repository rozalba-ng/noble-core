...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_CREATURE_OUTFIT, "SELECT entry, race, class, gender, skin, face, hair, haircolor, facialhair, head, shoulders, body, chest, waist, legs, feet, wrists, hands, back, tabard FROM creature_template_outfits WHERE entry = ?", CONNECTION_SYNCH);
}
